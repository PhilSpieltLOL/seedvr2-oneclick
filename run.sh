...
MODEL="${MODEL:-seedvr2_ema_3b_fp8_e4m3fn.safetensors}"
ATTENTION="${ATTENTION:-sageattn_3}"

blue() { printf "\n\033[1;34m==> %s\033[0m\n" "$*"; }

blue "Purging corrupt torchinductor cache from previous failed runs..."
rm -rf /tmp/torchinductor_root || true
...
# ninja: build backend that compiles flash-attn and SageAttention 3 from source.
pip install ninja
...
# SageAttention 2 (sageattn_2 backend; safe on any card).
pip install sageattention || true
# SageAttention 3 (Blackwell FP4, sageattn_3).
...
blue "Writing optimized webui.py"
cat > webui.py <<'PYEOF'
#!/usr/bin/env python3
import os
import shlex
...
# Aggressive memory management for extreme-batch VAE decoding
...
DEFAULT_MODEL = os.environ.get("MODEL", "seedvr2_ema_3b_fp8_e4m3fn.safetensors")
DEFAULT_ATTENTION = os.environ.get("ATTENTION", "sageattn_3")
...
ATTENTION_MODES = ["sageattn_3", "flash_attn_2", "sageattn_2", "sdpa"]
BATCH_CHOICES = [13, 17, 21, 25, 33, 41, 49, 65, 81, 97]
VRAM_STRATEGIES = [
    "Resident (fastest - model stays on the GPU)",
    "Resident + cached (offload DiT to CPU during the VAE phase; REQUIRED for big-batch decode without tiling)",
...
def build_command(input_path, output_dir, cuda_devices, dit_model, attention_mode,
    ...
    # --- Claude's torch.compile autotuner workaround ---
    _bootstrap = (
        "import runpy, sys\n"
        "try:\n"
    ...
    cmd = [sys.executable, "-u", "-c", _bootstrap, input_path]
    
    if output_dir: cmd += ["--output", output_dir]
    ...
    if int(max_resolution) > 0: cmd += ["--max_resolution", str(int(max_resolution))]
    cmd += ["--batch_size", str(int(batch_size))]
    if uniform_batch: cmd += ["--uniform_batch_size"]
    if int(temporal_overlap) > 0: cmd += ["--temporal_overlap", str(int(temporal_overlap))]
    if int(prepend_frames) > 0: cmd += ["--prepend_frames", str(int(prepend_frames))]
    if int(chunk_size) > 0: cmd += ["--chunk_size", str(int(chunk_size))]
    cmd += ["--color_correction", color_correction]
    if compile_dit: cmd += ["--compile_dit"]
    if compile_vae: cmd += ["--compile_vae"]
    
    if vram_strategy.startswith("Resident + cached"):
        cmd += ["--dit_offload_device", "cpu", "--vae_offload_device", "none", "--cache_dit"]
        ...
        if int(blocks_to_swap) > 0:
            ...
            if swap_io: cmd += ["--swap_io_components"]
    else:
    ...
    if ten_bit and video_backend == "ffmpeg": cmd += ["--10bit"]
    if extra_args and extra_args.strip(): cmd += shlex.split(extra_args)
    return cmd

def _newest_new_file(out_dir, before):
    if not os.path.isdir(out_dir): return None
    after = [os.path.join(out_dir, f) for f in os.listdir(out_dir) if os.path.isfile(os.path.join(out_dir, f))]
...
def run_upscale(video, server_path, output_dir, cuda_devices, dit_model,
    ...
    sep = "-" * 60
    before = ({os.path.join(out_dir, f) for f in os.listdir(out_dir)} if os.path.isdir(out_dir) else set())
...
with gr.Blocks(title="SeedVR2 Upscaler") as demo:
    gr.Markdown("## SeedVR2 Video Upscaler - RTX 5090 Extreme Speed Profile")
    with gr.Row():
        with gr.Column():
            video = gr.Video(label="Input video (or image)")
            server_path = gr.Textbox(label="...or path to a file already on the instance", placeholder="input/myvideo.mp4")
            output_dir = gr.Textbox(label="Output folder", value="output/")
            cuda_devices = gr.Textbox(value="0", label="CUDA devices ('0' = single GPU; '0,1' = use both GPUs)")
            dit_model = gr.Dropdown(DIT_MODELS, value=DEFAULT_MODEL, label="Model")
            attention_mode = gr.Dropdown(ATTENTION_MODES, value=DEFAULT_ATTENTION, label="Attention")
            color_correction = gr.Dropdown(["lab", "wavelet", "wavelet_adaptive", "hsv", "adain", "none"], value="lab", label="Color correction")
            with gr.Row():
                resolution = gr.Slider(480, 2160, value=1080, step=8, label="Target short-side resolution")
                max_resolution = gr.Slider(0, 3840, value=0, step=8, label="Max edge (0 = no limit)")
            batch_size = gr.Dropdown(BATCH_CHOICES, value=33, label="Batch size (4n+1; 33 is optimal for 32GB)")
            uniform_batch = gr.Checkbox(value=True, label="Uniform batch size (Stops late-stage OOMs)")
            extra_args = gr.Textbox(value="", label="Extra CLI flags (advanced)", placeholder="")
        with gr.Column():
            gr.Markdown("### Speed (attention + compile)")
            compile_dit = gr.Checkbox(value=True, label="torch.compile DiT (~20-40% faster)")
            compile_vae = gr.Checkbox(value=True, label="torch.compile VAE (Inductor fix applied)")
            gr.Markdown("### VRAM Management")
            vram_strategy = gr.Dropdown(VRAM_STRATEGIES, value=VRAM_STRATEGIES[1], label="VRAM strategy")
            blocks_to_swap = gr.Slider(0, 36, value=0, step=1, label="Blocks to swap")
            swap_io = gr.Checkbox(value=False, label="Swap I/O components")
            vae_tiling = gr.Checkbox(value=False, label="VAE tiling (OFF for extreme speed)")
            vae_tile_size = gr.Slider(256, 1024, value=1024, step=64, label="VAE tile size")
            vae_tile_overlap = gr.Slider(0, 256, value=64, step=16, label="VAE tile overlap")
            gr.Markdown("### Processing Params")
            chunk_size = gr.Slider(0, 1024, value=200, step=1, label="Chunk size")
            temporal_overlap = gr.Slider(0, 8, value=3, step=1, label="Temporal overlap")
            prepend_frames = gr.Slider(0, 8, value=4, step=1, label="Prepend frames")
            gr.Markdown("### Output")
            video_backend = gr.Dropdown(["opencv", "ffmpeg"], value="ffmpeg", label="Video backend")
            ten_bit = gr.Checkbox(value=False, label="10-bit x265")
    ...
    run_btn.click(
        run_upscale,
        inputs=[video, server_path, output_dir, cuda_devices, dit_model, attention_mode, resolution, max_resolution, batch_size, uniform_batch, temporal_overlap, prepend_frames, chunk_size, vram_strategy, blocks_to_swap, swap_io, compile_dit, compile_vae, vae_tiling, vae_tile_size, vae_tile_overlap, color_correction, video_backend, ten_bit, extra_args],
